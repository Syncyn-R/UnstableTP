...
def read_pdb_symbols(file_path) -> list:
    ...
    for index, line in enumerate(lines):
        if "?" in line and "Z" in line:
            ...
            pdb_symbols.append(line[43:-1])
    printc(f"Loaded {len(pdb_symbols)} symbols from pdb file ({str(time.time()-start_time)[:5]})s!", colorama.Fore.GREEN)
    return pdb_symbols
        ...
        with open(file_path, "r", encoding=get_file_encoding(file_path)) as source_file:
            ...
            for index, line in enumerate(lines):
                ...
                if not re.search(r'[^a-zA-Z0-9@?_$]', line[str_begin:str_end]):
                    plugin_symbols[line[str_begin:str_end]] = f"{file_path}:{index+1}"
        if verbose_mode:
            print(f"Processed file: {file_path}")
    printc(f"Loaded {len(plugin_symbols)} symbols from source code files {timer(start_time, time.time())}!", colorama.Fore.GREEN)
...
def compare_tsk(pdb_symbols, plugin_symbols, verbose_mode, similar_ratio, conn):
    ...
    for symbol in plugin_symbols:
        ...
        if symbol not in pdb_symbols:
            ...
            conn.send({'pos': plugin_symbols[symbol], 'sym': symbol, 'keywords': keywords, 'similar_syms': similar_symbols[:10], 'time': f"{timer(start_time, time.time())}"})
        elif verbose_mode:
            conn.send({'pos': plugin_symbols[symbol], 'sym': symbol, 'keywords': keywords,'time': f"{timer(start_time, time.time())}"})
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("-s", "--similar-ratio", type=float, default=0.8,help="After the symbol is not found, a lookup is attempted using the string similarity algorithm, where a smaller value may provide more results, possible values: 0 to 1")
    ...
    pbar = tqdm(total=len(plugin_symbols), desc="Progress",leave=True, unit="sym", unit_scale=False)
    plugin_symbols_tasks = dict_split(plugin_symbols, mp.cpu_count())
    parent_conn, child_conn = mp.Pipe()
    for plugin_symbols_task in plugin_symbols_tasks:
        mp.Process(target=compare_tsk, args=(pdb_symbols, plugin_symbols_task, verbose_mode, similar_ratio, child_conn), daemon=True).start()
    missing_count = 0
    for index in range(len(plugin_symbols)):
        result = parent_conn.recv()
        if len(result) == 4:
            pbar.write(f"{colorama.Fore.GREEN}Found symbol at {result['pos']} {result['time']}:")
            pbar.write(f"{result['sym']}{colorama.Fore.RESET}")
            pbar.write(f"{colorama.Fore.MAGENTA}Key words: {', '.join(result['keywords'])}{colorama.Fore.RESET}")
        elif len(result) == 5:
            similar_syms = '\n'.join(result['similar_syms'])
            pbar.write(f"{colorama.Fore.RED}Missing symbol at file {result['pos']} {result['time']}{colorama.Fore.RESET}")
            pbar.write(f"{colorama.Fore.YELLOW}{result['sym']}{colorama.Fore.RESET}")
            pbar.write(f"{colorama.Fore.MAGENTA}Key words: {', '.join(result['keywords'])}{colorama.Fore.RESET}")
            pbar.write(f"{colorama.Fore.GREEN}Similar symbols in symbol file:{colorama.Fore.RESET}")
            pbar.write(f"{colorama.Fore.LIGHTBLUE_EX}{similar_syms}{colorama.Fore.RESET}\n")
            missing_count += 1
        if index == len(plugin_symbols) - 1:
            color = colorama.Fore.LIGHTRED_EX
            if missing_count == 0:
                color = colorama.Fore.LIGHTGREEN_EX
            pbar.write(f"{color}Missing {missing_count} symbols{colorama.Fore.RESET}")
        pbar.update(1)
